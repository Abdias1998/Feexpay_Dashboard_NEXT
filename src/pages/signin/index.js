  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await signIn('credentials', {
        ...
        redirect: false,
        rememberMe: formData.rememberMe // Pass rememberMe to the credentials provider
      });
      ...
      if (result.error) {
        ...
        // Store authentication state based on rememberMe preference
    ...
    <div className="min-h-screen flex flex-col md:flex-row m-4 rounded-[40px] ">
      ...
      <div className="flex-1 flex items-center justify-center  p-8 xs:rounded-[40px] md:rounded-bl-none md:rounded-tr-[40px] md:rounded-br-[40px] shadow">
        <div className="w-full max-w-md ">
          ...
          <h2 className="text-lg font-bold mb-6">Connectez-vous</h2>
...
<div className="space-y-2">
  ...
  {errors.password && <p className="text-red-500 text-sm">{errors.password}</p>}
            ...
            <div className="text-center text-sm text-gray-600">
              ...
              <Link href="/sign-up" className="text-primary hover:text-orange-500">
                Créer un compte
              </Link>
            </div>
          </form>
